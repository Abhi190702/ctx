...
export function ProjectCard({ project }: { project: any }) {
  return (
    <Card className="transition-colors duration-150 hover:border-mint/40">
      ...
      <p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-400">{project.description || "Project memory workspace."}</p>
      <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-slate-500">
        <span>{project.capsules?.length ?? 0} capsules</span>
        <span>Updated {formatDate(project.updatedAt)}</span>
        {project.repository ? (
          <a href={project.repository} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 hover:text-white">
